...
function getPolynom(...args) {
  const polynomFunc = function func(x) {
    let maxDegree = args.length - 1;
    let result = 0;
    args.forEach((el) => {
      result += el * (x ** maxDegree);
      maxDegree -= 1;
    });
    return result;
  };
...
function retry(func, attempts) {
  return () => {
    try {
      ...
      return (!attempts) ? attempts : retry(func, attempts)();
    }
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argToPrint = args.map((el) => JSON.stringify(el));
    const script = `${func.name}(${argToPrint})`;
    logFunc(`${script} starts`);
